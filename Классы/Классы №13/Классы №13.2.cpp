...
int main()
{
	...
	Tdeque::iterator i;
	cout << "Ищем максимальный элемент и добавляем его в конец стека:\n";
	i = max_element(d.begin(), d.end());
	Pair max = *i;
	d.push_back(max);
	s = copy_deque_to_stack(d);
	cout << "Стек после изменения:\n";
	print_stack(s);
	...
	cout << "Стек после удаления элементов, совпадающих с ключом:\n";
	d.erase(remove_if(d.begin(), d.end(), Equal_s()), d.end());
	s = copy_deque_to_stack(d);
	print_stack(s);

	cout << "\nСреднее значение элементов стека: " << srednee(d) << '\n';
	p = srednee(d);

	d = copy_stack_to_deque(s);
	for_each(d.begin(), d.end(), Foo);
	s = copy_deque_to_stack(d);
	cout << "Стек после прибавления к каждому его элементу среднего значения:\n";
	print_stack(s);
	return 0;
}